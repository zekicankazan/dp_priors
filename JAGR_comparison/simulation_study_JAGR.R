source("functions.R")
source("JAGR_function.R")

n_vec <- round(10^seq(1,3, 0.1)); eps_vec <- c(0.2, 2); 
bdd_vec <- c(FALSE, TRUE); mu_vec <- c(0.1, 0.5)
grid <- expand.grid(n = n_vec, eps = eps_vec, bdd = T, mu = mu_vec)
reps <- 10000; nsims <- 20000

len <- nrow(grid)
plot_df_JAGR <- mutate(grid, coverage = NA, CI_len = NA, RMSE = NA)
for(num in 1:len){
  ...
  for(i in 1:reps){
    ...
    out <- Gibbs_JAGR(nsims, n, priv_Y_bar, priv_S_sq, eps_1, eps_2, flat_prior = T, 
                 bounded)
  ...
  plot_df_JAGR$RMSE[num] <- sqrt(mean((df$posterior_mode - mu)^2))
}