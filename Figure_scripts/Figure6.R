...
add_points_lm_left <- data.frame(name = "Posterior Mode", bounded = c(T, F),
                                 YT1 = c(posterior.mode(l_bd$S[,3], adjust = 1), 
                                         posterior.mode(l$S[,4], adjust = 1)),
                                 YTY = c(posterior.mode(l_bd$S[,5], adjust = 1), 
                                         posterior.mode(l$S[,6], adjust = 1))) %>%
  rbind(data.frame(name = "Confidential Values", bounded = c(T,F), 
                   YT1 = XTY_true[1,1], YTY = YTY_true)) %>%
  mutate(bounded = factor(bounded, c(F, T), c("Unconstrained", "Constrained")),
         name = factor(name, levels = c("Confidential Values",
                                        "Posterior Mode")))
...
p1 <- rbind(mutate(as.data.frame(l$S[,2:6]), bdd = F),
  ...
  geom_ribbon(data = df_fill_dat, aes(ymin = ymin, ymax = ymax), fill = "gray", 
              alpha = 0.5) +
  geom_point(aes(x = YT1, y = YTY, shape = name, color = name), size = 3,
             data = dplyr::filter(add_points_lm_left, bounded == "Unconstrained")) +
  scale_shape_manual(values=c(17,18)) +
  scale_color_manual(values = c("#7CAE00", "#00BFC4")) +
  labs(x = expression("Y"^"T"*"1"), y = expression("Y"^"T"*"Y"), color = "", shape = "") +
  theme_tufte() +
...
add_points_lm_right <- data.frame(name = "Posterior Mode", bounded = c(T, F),
                                  Intercept = c(posterior.mode(l_bd$theta[,1], adjust = 1), 
                                                posterior.mode(l$theta[,1], adjust = 1)),
                                  Slope = c(posterior.mode(l_bd$theta[,2], adjust = 1), 
                                            posterior.mode(l$theta[,2], adjust = 1))) %>%
  rbind(data.frame(name = "Confidential Values", bounded = c(T,F), 
                   Intercept = (solve(XTX_true)%*% XTY_true)[1,1], 
                   Slope = (solve(XTX_true)%*% XTY_true)[2,1])) %>%
  mutate(bounded = factor(bounded, c(F, T), c("Unconstrained", "Constrained")),
         name = factor(name, levels = c("Confidential Values",
                                        "Posterior Mode")))
...
p2 <- rbind(mutate(as.data.frame(l$theta), bdd = F), 
  ...
  facet_grid(bdd~.) +
  xlim(-4,4) +
  labs(x = expression(theta[1]), y = expression(theta[0]), color = "", shape = "") +
  geom_ribbon(data = df_fill_pars, aes(ymin = ymin, ymax = ymax), fill = "gray", alpha = 0.5) +
  geom_point(aes(x = Slope, y = Intercept, shape = name, color = name), size = 3,
             data = dplyr::filter(add_points_lm_right, bounded == "Unconstrained")) +
  scale_shape_manual(values=c(17,18)) +
  scale_color_manual(values = c("#7CAE00", "#00BFC4")) +
  theme_tufte()
...
## COMPUTE ADDITIONAL QUANTITES REFERENCED IN SEC 6
